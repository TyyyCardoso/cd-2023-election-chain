...
public class MinerP2P {
    ...
    public void startMining(String data, int difficulty, int numThreads) {
        ...
        for (int i = 0; i < numThreads; i++) {
            MinerTHR worker = new MinerTHR(ticket, nonce, difficulty, data, listener);
            workers.add(worker);
            worker.start();
        }
    }
    ...
    public boolean isMining() {
        ...
        // Verifica se alguma thread está ativa e se nonce ainda é 0
        return workers.stream().anyMatch(Thread::isAlive) && nonce.get() == 0;
}
