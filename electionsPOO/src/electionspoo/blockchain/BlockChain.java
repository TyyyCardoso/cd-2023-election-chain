...
public class BlockChain implements Serializable {
     ...
     */
    public void add(String data, int difficulty, MerkleTree tree) {
        //hash of previous block
        String prevHash = getLastBlockHash();

        // Get the number of available processors (you can modify this if needed)
        int numberOfCores = Runtime.getRuntime().availableProcessors();

        // Start the mining process
        Miner.startMining(prevHash + data, difficulty, numberOfCores, nonce -> {
            System.out.println("Mining done. Nonce: " + nonce);

            //build new block
            Block newBlock = new Block(prevHash, data, nonce, tree);

            //add new block to the chain
            chain.add(newBlock);

            System.out.println(chain.toString());
            // If you need to update the GUI or notify some other parts of your system after adding the block, you can do it here.
        });
    }
